...
class PathParser:
    ...
    @staticmethod
    def _flatten_path(parts: list[str], hw_dir: str, filename: str) -> str:
        """hw_dir 이후의 경로를 플래트닝
        ...
        try:
            ...
            logging.warning(f"과제 디렉토리를 찾을 수 없음, 원본 파일명 사용: {filename}")
            return filename
    
    @staticmethod
    def parse_path(file_path: str) -> tuple[str, str, str, str, str]:
        """파일 경로에서 필요한 정보 추출
        ...
        try:
            ...
            # 파일명 플래트닝
            flattened_filename = PathParser._flatten_path(parts, hw_dir, path.name)
            
            logging.debug(f"경로 파싱 결과: class_div={class_div}, hw_dir={hw_dir}, student_id={student_id}, flattened_filename={flattened_filename}")
            return class_div, hw_dir, student_id, flattened_filename, file_path
            
        except Exception as e:
            logging.error(f"경로 파싱 실패: {file_path} - {str(e)}")
