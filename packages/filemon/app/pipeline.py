...
class FilemonPipeline:
    ...
    async def process_event(self, event: FilemonEvent):
        ...
        try:
            ...
            if event.event_type == "deleted":
                await self.snapshot_manager.create_empty_snapshot(event.source_path)
                return
            
            # 수정 이벤트는 통합 처리 흐름
            if event.event_type == "modified":
                # 1. 스레드풀에서 파일 읽기 및 검증
                future = self.executor.submit(self.read_and_verify, event.source_path)
    ...
    def read_and_verify(self, source_path: str):
        ...
        try:
            ...
            return SourceFileInfo.from_source_path(Path(source_path), settings.BASE_PATH), st_after, data
        finally:
            f.close()
